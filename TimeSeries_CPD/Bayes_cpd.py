...
def offline_changepoint_detection(data, prior_func,
                                  ...
                                  truncate=-np.inf):
    """
    Function for estimating the posterior distribution over the current “run length,” or time since
    the last changepoint, given the data so far observed. This can be done by integrating over the posterior
    distribution on the current run length to find the marginal predictive distribution.

    Keyword arguments:
    data                                -- the time series data
    prior_func/hazard                   -- a function given the likelihood of a changepoint given the distance to the last one
    ...
    :returns function returns three things:
    Q: the log-likelihood of data [t, n] (evidence)
    P: the log-likelihood of a datasequence [t, s], given there is no changepoint between t and s (prior)
    Pcp: the log-likelihood that the i-th changepoint is at time step t (aka the run length posterior)
    """

    n = len(data)
    Q = np.zeros((n,))
    g = np.zeros((n,))  # Probability mass function for time between two succesive points
    G = np.zeros((n,))  # Distribution function of distance between two succesive points
    ...
    P[n - 1, n - 1] = observation_log_likelihood_function(data, n - 1, n)  # predictive probabilities
    Q[n - 1] = P[n - 1, n - 1]
    ...
    for t in reversed(range(n - 1)):
        ...
        for s in range(t, n - 1):
            ...
            summand = P[t, s] + Q[s + 1] + g[s + 1 - t]  # log_growth_probs
            P_next_cp = np.logaddexp(P_next_cp, summand)  # CP probabilities
        ...
        Q[t] = np.logaddexp(P_next_cp, P[t, n - 1] + antiG)  # Calculate evidence
